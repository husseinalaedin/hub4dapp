...
import { IconX, IconCheck, IconAlertCircle, IconInfoCircleFilled } from "@tabler/icons-react";
...
export class G {
    ...
    static gmail() {
        this.open_tab('https://mail.google.com/mail/u/0/#inbox?compose=new','gmail')
        // window.open(, "_blank");
    }
...
export class IP {
    static async get_ip() {
        ...
        if (agent_ip && agent_ip != '') {
            ...
            if ((new Date).getTime() - agent_ip_o_s.stm <= 1000 * 60 * 60 * 4)//checked ip every 4 hours
    ...
    static async get_ip64() {
        try {
            const response = await this.fetchWithTimeout('https://api64.ipify.org/?format=json', {
                timeout: 2000
            });
            const data = await response.json();
            let ip = ''
